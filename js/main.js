...
const SENTENСES = ['Всё отлично!', 'В целом всё неплохо. Но не всё.', 'Когда вы делаете фотографию, хорошо бы убирать палец из кадра. В конце концов это просто непрофессионально.', 'Моя бабушка случайно чихнула с фотоаппаратом в руках и у неё получилась фотография лучше.', 'Я поскользнулся на банановой кожуре и уронил фотоаппарат на кота и у меня получилась фотография лучше.', 'Лица у людей на фотке перекошены, как будто их избивают. Как можно было поймать такой неудачный момент?!'];
...
const createComment = () => {
  ...
  const messageNumber = getRandomInteger(0, SENTENСES.length - 1);
  const nameNumber = getRandomInteger(0, NAMES.length - 1);

  return {
    id: commentId,
    avatar: `img/avatar-${avatarNumber}.svg`,
    message: SENTENСES[messageNumber],
...
const generatePhotoInfo = () => {
  ...
  const commentsCount = getRandomInteger(0, MAXIMUM_COMMENT_NUMBER);

  return {
    id: photoId,
    url: `photos/${photoId}.jpg`,
    description: DESCRIPTIONS[photoId - 1],
    likes: getRandomInteger(MINIMUM_LIKES_COUNT, MAXIMUM_LIKES_COUNT),
    comments: Array.from({length: commentsCount}, createComment)
  };
};


const generateObjects = () => Array.from({length: PHOTO_COUNT}, generatePhotoInfo);
generateObjects();
